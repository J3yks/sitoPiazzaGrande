...
function About() {
  const imageUrl = "/images/fotoEdu.jpg"; // Usa una variabile per il percorso
  const pdfPathR1 = "/documents/rendiconto_2021.pdf"; // Percorso relativo dalla cartella public
  const googleViewerUrlR1 = `https://docs.google.com/gview?url=${window.location.origin}${pdfPathR1}&embedded=true`;
  const pdfPathR2 = "/documents/rendiconto_2022.pdf"; // Percorso relativo dalla cartella public
  const googleViewerUrlR2 = `https://docs.google.com/gview?url=${window.location.origin}${pdfPathR2}&embedded=true`;
  const pdfPathR3 = "/documents/rendiconto_2023.pdf"; // Percorso relativo dalla cartella public
  const googleViewerUrlR3 = `https://docs.google.com/gview?url=${window.location.origin}${pdfPathR3}&embedded=true`;
  const pdfPathStat = "/documents/STATUTO_PIAZZAGRANDEAPS.pdf"; // Percorso relativo dalla cartella public
